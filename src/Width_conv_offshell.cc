...
ClassImp(Width_conv_offshell) 

	Width_conv_offshell::Width_conv_offshell(){}

	Width_conv_offshell::Width_conv_offshell(const char *name, const char *title, 
			RooAbsReal& _xreco,
			RooAbsReal& _mean,
			RooAbsReal& _width,
			RooAbsReal& _coupl,
...
{ 
	...
	while((func = (RooAbsArg*)coefIter->Next())) {
		if (!dynamic_cast<RooAbsReal*>(func)) {
			coutE(InputArguments) << "ERROR: :Width_conv_offshell(" << GetName() << ") funcficient " << func->GetName() << " is not of type RooAbsReal" << endl;
			assert(0);
		}
		_coefList.add(*func) ;
	}
...
Width_conv_offshell::Width_conv_offshell(const Width_conv_offshell& other, const char* name) :  
...
Double_t Width_conv_offshell::evaluate() const 
{ 
	double value = 0.;
	if(xreco>xreco_max)
		return 0;
...
Int_t Width_conv_offshell::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const
{
...
Double_t Width_conv_offshell::analyticalIntegral(Int_t code, const char* rangeName) const
{
	switch(code)
	{
