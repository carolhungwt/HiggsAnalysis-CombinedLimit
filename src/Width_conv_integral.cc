...
ClassImp(Width_conv_integral) 
	...
	Width_conv_integral::Width_conv_integral(const char *name, const char *title, 
			RooAbsReal& _xreco,
			RooAbsReal& _mean,
			RooAbsReal& _width,
			RooAbsReal& _coupl,
...
{ 
	...
	while((func = (RooAbsArg*)coefIter->Next())) {
		if (!dynamic_cast<RooAbsReal*>(func)) {
			coutE(InputArguments) << "ERROR: :Width_conv_integral(" << GetName() << ") funcficient " << func->GetName() << " is not of type RooAbsReal" << endl;
			assert(0);
		}
		_coefList.add(*func) ;
	}
	...
	for(int j=0;j<1400;j++){
		double xgenval = xgen_min+  j*(xgen_max-xgen_min)/1400.;
		...
		bkg_int += nbkg* bkgcont[j] * eff_bkgcont[j];
		for(int i=0;i<201;i++){
			double recv = xgenval-100+i;
			double gen =0;
			if(recv > xreco_min && recv<xreco_max){
				xrecocopy->setVal(recv);
				gen = dynamic_cast<const RooAbsPdf*>(_coefList.at(2))->getVal(*xrecocopy);
			}
				resoval [i][j] = gen;
		//		integral_store[j] += gen*(xgen_max-xgen_min)/1400.;
		}
	}
...
Width_conv_integral::Width_conv_integral(const Width_conv_integral& other, const char* name) :  
...
Double_t Width_conv_integral::evaluate() const 
{ 
	double value = 0.;
	if(xreco>xreco_max)
		return 0;
	...
	double genwid = (xgen_max-xgen_min)/1400.;
...
//			Double_t T2 = dynamic_cast<const RooAbsPdf*>(_coefList.at(1))->getVal(*xgencopy);
	...
	int bintail =80;
	if(xreco>500&&xreco<1000)
		bintail =100;
	else if(xreco>1000)
		bintail =100;

		int ireal = int((xreco-bintail/2.-xgen_min)/genwid);
		double startgen = ireal*genwid+xgen_min;
		//cout << startgen << "\t"<<xreco<<endl;	
	for(int i=0;i<bintail;i++){
		//double genval = xgen_min+  i*genwid;
		double genval = startgen +  i*genwid;
		...
		Double_t T2 = bkgcont[i+ireal]; 
		Double_t T3 = coscont[i+ireal]; 
		Double_t T4 = sincont[i+ireal];
		double eff_T2= eff_bkgcont[i+ireal]; 
		double eff_T1=eff_sigcont[i+ireal];
...
Int_t Width_conv_integral::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const
{
...
Double_t Width_conv_integral::analyticalIntegral(Int_t code, const char* rangeName) const
{
	switch(code)
	{
		...
		case 4: 
			{
				...
				double int_sig  = dynamic_cast<const RooHistFunc*>(_coefList.at(3))->getVal();
				double int_int  = dynamic_cast<const RooHistFunc*>(_coefList.at(4))->getVal();
				double integral = bkg_int+ int_sig*fabs(coupl) + sqrt(coupl)*int_int; 
