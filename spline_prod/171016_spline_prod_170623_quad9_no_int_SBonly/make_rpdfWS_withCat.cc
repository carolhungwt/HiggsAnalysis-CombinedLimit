...
//	gROOT->ProcessLine("gSystem->Load(\"./make_rpdfWS_withCat_cc.so\")");
...
//	CurTime <<put_time(&tm, "%d-%m-%Y %H-%M-%S");
//	auto TimeStr = CurTime.str();

//	char cmd[200];
//	sprintf(cmd, "echo %s 	%s Pts:%d 	%0.2f seconds >> %s", TimeStr.c_str(), tag2, totalpts, (double)(clock() - tStart)/CLOCKS_PER_SEC, Timefile.c_str()); 
//	system(cmd);

//	printf("Time Taken: %0.2f seconds",(double)(clock() - tStart)/CLOCKS_PER_SEC);
...
//		cout<<dim[2]<<endl;
		...
		m4l.push_back(dim[1]);	sigmas.push_back(dim[0]);	fcns.push_back(fcn);
    		/*if(fcn>0.1)*/ //   cout << dim[1]<<" "<<dim[0]<<" "<<fcn<<" \n";
				points.push_back(splineTriplet_t(dim[0], dim[1], fcn));
				}
		}
	spFac -> setPoints(points);
//	spFac -> setPoints(m4l,sigmas,fcns);
	RooNCSpline_2D_fast* spPDF = spFac->getFunc();
	spPDF->SetNameTitle(tag,tag);
//	RooPlot* plot = mreco->frame();
//	TCanvas* c1;
//	c1->cd();	
//	spPDF->plotOn(plot);
//	plot->Draw();
//	c1->SaveAs("~/www/check_r1.png");
